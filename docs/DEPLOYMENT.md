...
The control-plane image is built from `tsconfig.runtime.json`, which excludes tests and `src/test-support`. CI verifies both the runtime distribution and final image contain no test authority markers. Test-only schema identities and provider fixtures are never packaged as production authority material.
