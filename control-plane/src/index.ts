...
export const createControlPlane = (repository = new MemoryRepository(), webhookSecret = process.env.TALOS_WEBHOOK_SECRET, options: { adminToken?: string; sweepIntervalMs?: number } = {}): ControlPlaneServer => {
  ...
  const service = new TaskService(repository, scheduler, profiles, signer, { onWebhook: (event, callback) => dispatcher.dispatch(event, callback) });
  const server = createApiServer(service, repository, { adminToken: options.adminToken, clock: Date.now }) as ControlPlaneServer;
